...
class PlacesViewModel {
    ...
    if (selectArgs.cityId != null) {
      return _placeUseCase
          .getPlacesBySelectArgs(
              SelectArgs(placeType: PlaceType.city, id: selectArgs.cityId))
          .map((event) => event.first.title);
    } else if (selectArgs.id != null) {
      return _placeUseCase
          .getPlacesBySelectArgs(selectArgs)
          .map((event) => event.first.title);
    } else if (selectArgs.placeType == PlaceType.city) {
      return Future.value(AppLocalizations.of(_context)!.cities_title)
          .asStream();
    } else if (selectArgs.isFavorite == true) {
      return Future.value(
              AppLocalizations.of(_context)!.nav_menu_item_bookmarks)
          .asStream();
    } else if (selectArgs.isVisited == true) {
      return Future.value(AppLocalizations.of(_context)!.nav_menu_item_visited)
          .asStream();
    } else
  ...
  Stream<List<Place>> getPlaces() =>
      _placeUseCase.getPlacesBySelectArgs(selectArgs);
